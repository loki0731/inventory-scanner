        ...
        async with SessionLocal() as db:
            ...
            try:
                ...
                if asset.platform == "linux":
                    connection = await _run_linux_scan(
                        asset,
                        credential,
                        secret,
                    )
...
async def _run_linux_scan(
    ...
    """Создает и устанавливает SSH-соединение для Linux-сканирования."""
    ...
    try:
        logger.info(
            "Connecting to Linux host %s via SSH as %s",
            asset.ip_address,
            credential.username,
        )
        ...
        logger.info(
            "SSH connection established to Linux host %s",
            asset.ip_address,
        )
        ...
        raise ScanFailure(
            "UNREACHABLE",
            f"Failed to connect via SSH: {str(e)[:200]}",
        )
...
def _create_winrm_connection(
    ...
    """Создает WinRM-соединение для Windows-актива."""
    ...
    # Формируем корректное имя Windows-учетной записи.
    #
    # Локальная учетная запись:
    #   .\inventory-scanner
    #
    # Доменная учетная запись:
    #   DOMAIN\inventory-scanner
    #
    # UPN:
    #   inventory-scanner@example.local
    ...
    logger.info(
        "Creating WinRM connection: host=%s port=%s https=%s username=%s",
        asset.ip_address,
        port,
        use_https,
        username,
